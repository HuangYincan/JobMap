...
- OTP send (2026-08-21): email goes out for real via Resend (`POST /api/auth/otp/send` → `{ ok, provider, expiresAt, messageId }`; needs `RESEND_API_KEY`, error mapping & retry policy see tech/25). Phone stays a demo stub (`demo: true, hint: '000000'`), keep the hook for a real SMS provider later.
- OAuth (2026-08-22): real authorization code flow for GitHub / Google / WeChat — `GET /api/auth/oauth/providers` (probe `configured`), `GET /api/auth/oauth/start?provider=&next=` (302 to the provider, signed `oauth_state` cookie), `GET /api/auth/oauth/callback/<provider>?code=&state=` (state check → code exchange → userinfo → `upsertIdentity` → session cookie → 302 back to `next`); failure → 302 `?auth_error=oauth_state_invalid|oauth_provider_error`. Unconfigured credentials → frontend falls back to demo login (`POST /api/auth/oauth` stub unchanged, `demo: true`). Endpoint detail / error mapping / ⚠️ manual config checklist: tech/27. Do not add X. Existing `'x'` account rows stay valid.
