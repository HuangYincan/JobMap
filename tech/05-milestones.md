...
- Account slice (2026-08-16, revised 2026-08-29): default map mode is **work**; Settings rail item removed into Profile L2 (career prefs + notifications + avatar crop); Recent L2 is application watch (`/api/me/applications`, user-editable stages in `preferences.applicationPipeline`; default 已投递 / 面试中 / Offer / 未通过 / 已撤回 / 已接受). Search history still persists (`/api/me/search-history`; guests `dm.guest-search-history.v1`) but is not shown in Recent. Login is a split glass card (phone / email / other: GitHub Google WeChat). Migrations `005`–`007` plus `021` are in `db/migrations`. Demo APIs are in-memory until `DATABASE_URL` is wired. Seed logos go through `resolveCompanyLogo` (career-site icon → company icon → emoji).
