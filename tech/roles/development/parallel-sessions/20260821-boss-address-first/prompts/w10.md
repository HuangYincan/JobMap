# Workstream w10 — fix/geocode-grader-relax(office POI 匹配放宽:限定词组合序列)

## 你的身份

boss 派发的 headless 开发 worker。**只在本 worktree 内开发,不 merge、不 push、不碰主树。**
汇报写入批次目录 reports/w10.md(末两行 token,见文末)。

## 背景(2026-08-22 r4 apply 实测)

r4 apply(城市中心重跑)后仍有 **941 站地址是多城市占位串**(「北京/上海/深圳/成都」),
这些站 city/province 干净(如 `58集团-site-shanghai` city=上海市),走公司名 place-search
分支,但 **831 站 no-result**。活体诊断(真实 key,上海,query=百度)证明根因:
...
3. **测试**(`server/tests/site-geocode.test.mjs`,对齐现有风格):
   ...
   - 复合拆分的边界:研发大厦 拆 研发|大厦(不是 研发大|厦);科技公司 拆 科技|公司;
     超长串(4+ token)不崩。
   - 若既有测试断言旧严格行为,更新为新语义(与 w2 先例一致,更新时在汇报列出)。
4. **只读统计**(不跑网络):跑 `node scripts/plan-site-geocode.mjs` 或等价方式给出
   r5 预期尝试数(needs 当前 1248,不要求精确——给出数量级即可)。
5. 门禁:`cd /Users/acccan/dm-wt-grader/server && npm test`(基线 1370 pass/0 fail/2 skip,
   全绿)、`npm run typecheck`;`cd /Users/acccan/dm-wt-grader && make docs-check`、
   `git diff --check`。

## 文件边界

- **只允许改**:`server/src/lib/site-geocode.ts`、`server/tests/site-geocode.test.mjs`。
- **不碰**:scripts、数据文件、其他模块。

## 汇报

写 `/Users/acccan/domain-map/tech/roles/development/parallel-sessions/20260821-boss-address-first/reports/w10.md`。
**末两行必须精确**:
...
Conventional Commits(如 `fix(geocode): accept compound qualifier suffixes in office POI name match`)。不 merge、不 push。
