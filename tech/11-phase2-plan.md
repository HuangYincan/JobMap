...
- [x] `GET /api/pois` - POI 列表（`loadServerCatalog`：严格 DB-only，无库 → 502；seed 已归档 `tech/backup/seed-data`）
...
- [x] 公开读走导入行（`loadWorkCatalogFromDb` → `loadServerCatalog`；严格 DB-only，无库 → 502）
...
- [ ] 页面加载时间 LCP < 2.5s — **生产构建实测**（`next build` + `next start`，2026-08-17）：FCP 228ms / load 219ms / TTFB 24ms / CLS 0；LCP 无法经父页读取（地图是 iframe），但首屏绘制远低于目标
- [ ] 搜索防抖延迟 300ms
- [ ] 虚拟滚动支持 1000+ 卡片流畅滚动
- [x] 测试覆盖率 > 70% — **2026-08-17 实测**：`npm run test:coverage`（Node 内置覆盖率）78.96% 行 / 77.62% 分支 / 75.34% 函数
