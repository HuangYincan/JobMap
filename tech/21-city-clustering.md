...
无法点选,视觉是"一坨"。
(注:以上为历史 LOD 密度下的动机描述;2026-08-25 起个体层全量展示
无 tier 过滤,全国/省级视野的密度管理完全由本聚合承担——见规则 7。)
...
5. 与现有交互零冲突:选中高亮、个体 pin 显示密度(2026-08-25 前为
   `tier <= zoom` LOD 过滤,只属 zoom > 8 个体层;2026-08-25 用户裁定起
   个体层全量显示,无 tier 过滤)全部在聚合层之外工作——聚合层不做 LOD,
   计数与 zoom 无关(见规则 7)。
...
7. **计数口径(2026-08-20 修订,替代 w1 版)**:徽章 N = 该城市**全部**公司数
   ...
   个体 pin 显示密度,聚合区间不参与(2026-08-25 起个体层也已无 tier 过滤,
   全量展示,见 tech/18 §2.2 修订)。计数 = 池内容:work 模式全量加载后
   池恒定(见 tech/16「2026-08-20:首点刷新」条目),徽章数与导航历史/缩放
   路径无关。分组键与徽章标签用裸城名(`bareCityName`,去省/市/区后缀):
   DB 中「杭州市」/「杭州」并存(102+50 站点),不归一会出现同一城市两个
   徽章。下钻 zoom 11 后全量个体 pin 出现(2026-08-25 起与徽章同口径:
   该城全部公司),数量与徽章 N 一致——不再是「LOD 放大涌现」式多于徽章。
...
- `server/src/components/map-shell.tsx`:work 模式 zoom ≤ 8 时按 pipeline
  ...
  裁剪,见 tech/16),徽章 N 与 zoom 无关;个体层全量显示(2026-08-25 修订:
  工作 LOD 取消,下钻后 tier 12 未打标公司也出现;渲染层第二种模式)。
