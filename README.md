...
- Database `db/`: ordered PostGIS migrations `001`–`019`(`011` national scope / `012` tier+category / `013` hz_pois / `014` credentials / `015` recent / `016` site-key / `017` avatar / `018` memories / `019` memory uniqueness)。Live apply verified in the ledger, `make test-integration` passes。`npm run import:seed:apply` 首次 live-write 137 companies / 137 sites / 240 open positions(2026-08-17 杭州 pilot,含 radar + portals);后续增量/全量 apply 属用户操作(2026-08-23 dry-run 计划见上,apply 后实入行数待回填)。
