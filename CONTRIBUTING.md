...
Server commands (`cd server`): `npm test` (**1689 tests / 1686 pass / 0 fail / 3 skip, 2026-08-27, commit `d899b3f` snapshot**), `npm run typecheck`, `npm run dev` / `build` / `start`. Data commands that touch Postgres (`npm run import:seed:apply`, `geocode:sites:apply`, `audit:pins`, `import:hz:pois:apply`) need `DATABASE_URL` from `server/.env.local` (never print or commit it) and, where noted, `AMAP_WEB_KEY` (geocode also accepts `BAIDU_MAP_AK` / `TENCENT_MAP_KEY` fallbacks).
