...
Develop Next.js 16 + React 19 components following Domain Map's architecture principles.
...
3. **Read Existing Patterns**
   ...
   - Primary rail is collapsed-first: no brand wordmark. Search is a navItem button (same 42px hit as Layers), not a filled chip and not an input; click toggles Explore L2 (`openRail("explore")`). Desktop rail has no Explore item. Typing lives in SecondarySidebar / the mobile drawer. The menu control uses a sidebar glyph when collapsed and a chevron when open, and slides to the right of the rail as it expands.
   ...
   - Job detail is a third panel (`jd-panel.tsx`) in the same flex cluster as Explore, 6px to the right. Do not nest JD inside the secondary sidebar, and do not absolutely position it at a fixed left (that clips on narrower windows). On viewports ≤767px the desktop rail and L2/L3 clusters hide. The bottom drawer owns search, ModeSwitcher, FilterPanel, SortSelector, POIList, POIDetailView, JdPanel, and SavedList (same compare table as desktop Layers L3) stacked in the same `--soft-strong` sheet (mini/half/full). Half/full: avatar on the right of `mobileToolbar` opens the account sheet (Profile + Layers / Recent). The `mobileToolbar` left cluster is ModeSwitcher + four icon buttons (Layers / Explore / Recent / AI assistant, i18n `aria-label` + `title`, 40px touch target, active blue `#007AFF`, same `aria-pressed` pattern as the desktop rail). Layers / Recent open their sheets in the full drawer (Recent guests open login); overlay guests open login from the Layers sheet; Explore just switches the sheet back to the default; AI opens a drawer-embedded agent sheet (`mobileSheet === "agent"`, full drawer) — same sheet pattern as Layers / Recent, not a separate floating overlay: the toolbar item sets `mobileSheetBack("explore")` + `mobileSheet("agent")` + `setDrawer("full")`, and `AgentPanel` renders with `embedded` (position static, fills the sheet body; message list scrolls internally, input pinned at the sheet bottom). The floating ball **and its anchored AgentPanel** are hidden ≤767px (`.panel { display: none }`; the old full-width bottom-sheet overlay with z-index 13 is removed), so the toolbar item is the only mobile AI entry — `agentOpen` drives the desktop ball only. Desktop AgentPanel chrome is ✦ 助手 + clear/undo/close (no session/memory popovers; no expand). Tapping an already-active item returns to Explore (mirrors the avatar re-tap). Back buttons in Layers / Recent / Agent return to their source via `mobileSheetBack` (toolbar entry → Explore; account sub-nav entry → account). Mini: avatar to the right of the search field expands to full account. Tapping the avatar again while already on the account sheet returns to Explore. Embedded Profile / Recent keep a visible close (do not hide `styles.close` when `embedded`). Account / Layers also expose `mobileBackBtn`. Preference cards in the drawer must be fluid (`max-width: 100%`, `min-width: 0`); `.sheet` must come after `.sidebar` so `width: 100%` wins over the desktop 380px. Mobile search suggestions appear only in half/full (`drawer !== "mini"`). They are an absolute liquid-glass overlay (`mobileSearchStack` + `mobileSuggestions`) over the list, never an in-flow block that pushes chips/results down, and never a sibling in the input flex row. Clicking a job replaces the company page with `JdPanel` in the drawer — do not `display: none` the JD panel at 767px. Avatar crop opens via `createPortal(..., document.body)` so Profile’s `pointer-events: none` cluster cannot swallow it. Do not mount a second desktop Explore on mobile.
   ...
   - Work Explore does **not** show commute-filter chrome (origin located / walk-bike-transit-drive / max-minutes slider / strict-near-compare counts). The list is not commute-clipped. Explore does **not** show 岗位 / 对比 / 行程 tabs (`WorkExploreTabs` removed 2026-08-31). Cards do not show straight-line estimate minutes or a compare checkbox. Agent `navigation__filterByCommute` is unchanged. Company compare remains in Layers L3.
   ...
   - Domain POIs accumulate in a catalog (soft cap 300). Panning the map does not refetch. Each mode’s catalog lives in browser `sessionStorage` (`lib/mode-cache.ts`) — switching Domain ↔ Work restores the pool and does not re-hit AMap. Only the blue refresh icon clears that mode’s cache and researches. **When curated data changes (coordinate fixes, import refreshes), bump `MODE_CACHE_VERSION` so stale session caches invalidate and refetch.** Result header: blue refresh icon next to the count; blue plus icon on the right adds ~300 more points. Job POI card **display** distances use the user location, falling back to the view center only if geolocation is missing. Card **sort**, the distance filter, and the buffer circle stay on the view center (`distanceOrigin = mapCenter`). When the distance slider is set, draw a brand-blue `#007AFF` buffer circle on that view-center origin (`distanceFilterMeters`) with an east-edge handle that resizes the radius and snaps back to the 0.5km slider. Search is one `searchNearBy` from the user location by default; refresh pins the origin to the current view center. Radius is map-scale × 30 (over 50km falls back to AMap default 3000m). Serialize at 3 req/s. Do not fan out a 16-cell grid.
   ...
   - Login is a centered split glass card (animated overlay orbs + left promo, close X + click outside). Method tabs are 手机 / 邮箱 / 其他登录 only — **no password tab** (2026-08-30). Selected = blue underline, hover = lighter blue without underline. On ≤720px hide the promo/logo, keep tabs left-aligned with larger gap and vertical dividers, 44px tap. OTP send is a text button inside the input. Primary button is green 登录. Other is one full-width icon+label row each: GitHub / Google / WeChat via `POST /api/auth/oauth` — do not add X. Password set/change lives in Profile「密码与安全」; `/api/auth/password/login` and `/register` stay as APIs, not login-card UI. OTP 已真发:email 经 Resend、phone 经阿里云短信(2026-08-22,tech/25/26);缺配置时发送接口 503 `EMAIL_NOT_CONFIGURED` / `SMS_NOT_CONFIGURED`(优雅降级)。 Never print or commit `.env` secrets. Green is allowed on Sign in / Update Profile / Save crop; everywhere else keep `#007AFF` chrome (small text uses `--blue-ink`).
   - Recent L2 is **application watch** (`applications` / `/api/me/applications`). JD Apply (`onApply`) records the click then follows the real URL in a new tab; do not steal focus to Recent. Guests who tap Apply are asked to sign in (the careers URL is not opened). Jobs with no apply URL use 加入监视 (write only). Rows live in Recent, not a fourth panel. Signed-in users can add a row in L2 or import/export UTF-8 CSV (`公司,岗位,阶段,投递链接,投递时间` or English aliases; `POST /api/me/applications/import`, max 200). Manual/CSV ids are `manual:` hashes of normalized company+title. Add/import merge POST `{ item }` / `{ items }` into the list immediately (`reconcileApplications`); a failed write keeps the composer open. Manual rows with a URL open it in a new tab; rows without a catalog id do not switch to Explore. Profile L2 shows a count jump (`N 条投递 · 在最近里看`). Guests who tap Recent open login (same as the saved overlay toggle). Stages are user-editable (`users.preferences.applicationPipeline` via `PUT /api/me/applications/pipeline`); `PATCH /api/me/applications` updates one row. Defaults: 已投递 / 面试中 / Offer / 未通过 / 已撤回 / 已接受. Watch chips are 全部 / 进行中 / 已结束. Unmodified legacy 12-stage catalogs collapse on read (等面/一面/二面/三面 → 面试中; 一面挂/二面挂/三面挂 → 未通过). Users can add, rename, regroup, and remove stages (keep ≥1). Offer / 已接受 pills use semantic green; 未通过 (and legacy 一面挂/二面挂/三面挂) use `#ff3b30`; other closed stages use muted; active stages use brand blue. Each watch row has an × (`DELETE /api/me/applications?id=`) that removes it immediately with no confirm. Search history still writes persistable queries (`/api/me/search-history`; guests `dm.guest-search-history.v1`, cap 30) but is **not** shown in Recent. Domain / AMap queries never record. Login still uploads persistable guest search rows.
   - Saved is the same persistability gate (`saved_places` / `/api/me/saved`). Only recruitment catalog POIs (`isPersistablePoi`). Domain AMap bookmarks are hidden; POST returns 400 `NOT_PERSISTABLE`. There is **no independent Saved rail or toolbar item**. Guests who tap a persistable bookmark or the overlay toggle open login; never invent a local cloud list. Company compare lives **inside Layers L3** (`SavedPanel` nested beside Layers L2): turning the overlay on shows the list + pick-two table (`lib/compare-saved.ts`, catalog/seed first, snapshot fallback). Closing the L3 card turns the overlay off. On mobile the same `SavedList` stacks under the overlay toggle in the Layers sheet. Layers L2 (`layers-panel.tsx`) keeps the overlay toggle (`lib/saved-overlay.ts`) and basemap styles. Overlay and style both persist in sessionStorage (`saved-overlay` / `map-style`). A user-picked style is not overwritten by system dark/light. Clicking a saved row opens detail via `resolveSavedForFly` (live catalog/seed first). Do not put the overlay switch on the rail itself, and do not keep a second basemap picker on the map chrome.
