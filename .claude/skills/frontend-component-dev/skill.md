...
3. **Read Existing Patterns**
   ...
   - Work Explore does **not** show commute-filter chrome (origin located / walk-bike-transit-drive / max-minutes slider / strict-near-compare counts). The list is not commute-clipped. Compare and trip stay on `WorkExploreTabs` (岗位 / 对比 / 行程). Cards may still show straight-line estimate minutes and a compare checkbox. Agent `navigation__filterByCommute` is unchanged.
