...
const QUALIFIER_SUFFIXES = new Set([
  ...
  // 2026-08-27 (fix/grader-region-prefix): 区域前缀 + 办公形态词是常见真实办公点名
  // (小米集团华东总部 / 华南研发中心 / 西南研究院 / 华北分公司)。此前「华东总部」
  // 拆不出 token (华东不在表内) → 整候选判 no → name-mismatch, 多城中心钉站点
  // 重 geocode 全被拒 (实测南京 35 站 0 解析, 小米集团华东总部亦被拒)。区域词
  // 之后仍须是办公形态 token (店/站/驿站/包装 等非限定词混入照旧拒)。
  '华东', '华南', '华北', '华中', '西南', '西北', '东北', '东南',
  // 2026-08-27 (fix/grader-legal-entity): 法律实体词是真实办公点常见的「公司名 + 实体
  // 后缀」形态 (帆软软件有限公司(南京运营中心) / 中信银行南京分行 / 宁波银行滨江支行 /
  // XX电子科技有限公司)。此前这些词不在表内 → 整候选判 no → name-mismatch, 有真实
  // 办公 POI 的公司也被拒 (实测帆软/银行类 5 城站全拒)。仍须 c.includes(q) + 城市/
  // regeo 闸门, 门店/驿站/包装 等非限定词混入照旧拒。
  '软件', '电子', '网络', '通信', '分行', '支行',
]);
