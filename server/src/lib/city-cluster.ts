...
// 的「缩放时聚合点数量变化」)。2026-08-25 修订(工作 LOD 取消):个体
// pin 也不再有 tier 过滤,聚合计数与个体显示口径已一致(全量展示)。
//
 ...
 * 2026-08-25 修订:工作地图已取消按 zoom 隐藏公司(LOD 客户端过滤退役,
 * tech/18 §2.2)——下钻后该城全部公司全量出现(含未打标 tier 12),
 * 不再「tier > 11 被客户端过滤隐藏」。
 */
 ...
 *   只有跨聚合↔个体边界(8.0→8.1)才切换计数口径——个体 pin 全量显示
 *   (2026-08-25 起无 tier 过滤,与徽章计数口径一致);
