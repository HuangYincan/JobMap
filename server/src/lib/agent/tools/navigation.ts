...
function orderResolvedPositions(
  ids: readonly string[],
  records: readonly WorkPositionDetailRecord[],
): WorkPositionDetailRecord[] {
  const byId = new Map<string, WorkPositionDetailRecord>();
  for (const record of records) {
    if (!byId.has(record.positionId)) byId.set(record.positionId, record);
  }
  return ids
    .map((id) => byId.get(id))
    .filter((record): record is WorkPositionDetailRecord => record != null);
}
    ...
    },
    {
      ...
      async call(input: Record<string, unknown>, ctx: AgentContext): Promise<ToolResult> {
        ...
        if (Array.isArray(input.destinations)) {
          ...
          const records = orderResolvedPositions(ids, await resolvePositions(ids));
