 ...
 * - logoUrl 已失败（favicon.im 静态跳过 / 预检失败）→ 依次试
 ...
 * - **链式预检（2026-08-23 ws-i；2026-08-30 静态跳过 favicon.im）**：
 *   toPreflight 只含候选链中**第一个 unknown**。favicon.im 在 icon-preflight
 *   里静态 fail（不 new Image），故首个 unknown 通常是 icon.horse——首会话
 *   不再对每家公司打一遍无 CORS 的 favicon.im。
