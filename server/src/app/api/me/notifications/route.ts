...
// 用进程内 Map（取舍：单实例部署有效、重启清零；不落 DB——account-store 是账号读写面，
// 每次 POST 多一次写库会把「扫描+入队」变成「扫描+写库+入队」，且本 WS 不碰 account-store）。
const NOTIFY_COOLDOWN_MS = 60_000;
const notifyCooldown = new Map<string, number>(); // userId → lastScanAt
...
export async function POST() {
  ...
  const now = Date.now();
  const last = notifyCooldown.get(user.id);
  ...
  // 惰性清理：map 超过阈值时顺手清掉已过期的条目（账号量小，纯防御）。
  if (notifyCooldown.size >= 1000) {
    const cutoff = now - NOTIFY_COOLDOWN_MS;
    for (const [id, at] of notifyCooldown) {
      if (at <= cutoff) notifyCooldown.delete(id);
    }
  }
  notifyCooldown.set(user.id, now);
